  ...
  if (!order) {
    return (
      <div className="mx-auto max-w-2xl rounded-2xl border border-dashed border-neutral-200 py-20 text-center">
        ...
        <Link href="/seller/orders" className="mt-4 inline-block text-13 text-neutral-400 underline underline-offset-2">
          Back to orders
        </Link>
      </div>
    );
    ...
    <div className="mx-auto max-w-2xl">
      ...
      <div className="mb-6 flex items-center gap-3">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-3">
            <h1 className="font-mono text-xl font-bold tracking-tight text-neutral-950">
              #{order.id.slice(0, 8).toUpperCase()}
            </h1>
            <OrderStatusBadge status={order.status} />
          </div>
          <p className="mt-0.5 text-13 text-neutral-500">{formatDate(order.createdAt)}</p>
        </div>
      </div>
      ...
      {!isCancelled && nextStatus && (
        <div className="mb-4 rounded-2xl border border-neutral-100 bg-white p-6">
          <h2 className="mb-4 text-sm font-semibold text-neutral-900">Update status</h2>
          <div className="mb-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg bg-neutral-950 px-4 py-2 text-13 font-medium text-white hover:bg-neutral-800 disabled:cursor-not-allowed disabled:bg-neutral-300 transition-colors"
            >
            ...
            {order.status !== 'cancelled' && order.status !== 'delivered' && (
              <button
                ...
                disabled={updateStatus.isPending}
                className="rounded-lg border border-red-200 px-4 py-2 text-13 font-medium text-red-500 hover:bg-red-50 disabled:opacity-40 transition-colors"
              >
                Cancel order
              </button>
            )}
      ...
      {order.trackingNote && (
        <div className="mb-4 rounded-2xl border border-neutral-100 bg-neutral-50 px-5 py-4">
          <p className="text-xs font-semibold uppercase tracking-wider text-neutral-400">Tracking note</p>
          <p className="mt-1 text-sm text-neutral-700">{order.trackingNote}</p>
        </div>
      )}
          ...
          {order.items?.map((item) => (
            <div key={item.id} className="flex items-center gap-4 py-3 first:pt-0 last:pb-0">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-neutral-400">Qty: {item.quantity} · {formatPrice(item.unitPrice)} each</p>
              </div>
              <p className="shrink-0 text-sm font-medium text-neutral-900">{formatPrice(item.lineTotal)}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Pricing */}
      <div className="mb-4 rounded-2xl border border-neutral-100 bg-white p-6">
        <h2 className="mb-4 text-sm font-semibold text-neutral-900">Pricing</h2>
        <div className="space-y-2">
          <div className="flex justify-between text-13 text-neutral-600">
            <span>Subtotal</span>
      ...
      {order.shippingAddress && (
        <div className="rounded-2xl border border-neutral-100 bg-white p-6">
          <h2 className="mb-3 text-sm font-semibold text-neutral-900">Ship to</h2>
          <div className="space-y-0.5 text-13 text-neutral-600">
