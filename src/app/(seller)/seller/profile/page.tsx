...
export default function SellerProfilePage() {
  ...
  return (
    <div className="space-y-8">
      <h1 className="text-[1.5rem] font-bold tracking-tight text-neutral-950">Profile</h1>

      {/* Account info */}
      <section className="rounded-2xl border border-neutral-100 bg-white p-6">
        <h2 className="mb-1 text-[15px] font-semibold text-neutral-950">Account information</h2>
        <p className="mb-6 text-[13px] text-neutral-400">{user?.email}</p>
        ...
        <form onSubmit={handleP(onProfile)} noValidate className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="flex flex-col gap-1.5">
              ...
              {errP.name && <p className="text-[12px] text-red-500">{errP.name.message}</p>}
            </div>
            <div className="flex flex-col gap-1.5">
              ...
              {errP.phone && <p className="text-[12px] text-red-500">{errP.phone.message}</p>}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            {errP.businessName && <p className="text-[12px] text-red-500">{errP.businessName.message}</p>}
          </div>
      ...
      {/* Change password */}
      <section className="rounded-2xl border border-neutral-100 bg-white p-6">
        <h2 className="mb-1 text-[15px] font-semibold text-neutral-950">Security</h2>
        <p className="mb-6 text-[13px] text-neutral-400">Change your account password.</p>

        <form onSubmit={handlePw(onPassword)} noValidate className="space-y-4">
          <div className="flex flex-col gap-1.5">
            <label className="text-[13px] font-medium text-neutral-700">Current password</label>
            <input {...regPw('currentPassword')} type="password" placeholder="••••••••" className={inputCls} autoComplete="current-password" />
            {errPw.currentPassword && <p className="text-[12px] text-red-500">{errPw.currentPassword.message}</p>}
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="flex flex-col gap-1.5">
              ...
              {errPw.newPassword && <p className="text-[12px] text-red-500">{errPw.newPassword.message}</p>}
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[13px] font-medium text-neutral-700">Confirm new password</label>
              <input {...regPw('confirmPassword')} type="password" placeholder="Repeat new password" className={inputCls} autoComplete="new-password" />
              {errPw.confirmPassword && <p className="text-[12px] text-red-500">{errPw.confirmPassword.message}</p>}
            </div>
          </div>
