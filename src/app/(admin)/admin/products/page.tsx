    ...
    <div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-neutral-200 py-20 text-center">
          ...
          <p className="text-15 font-medium text-neutral-500">No products yet</p>
        </div>
      ) : (
        <div className="overflow-hidden rounded-2xl border border-neutral-100 bg-white">
          ...
          <div className="grid grid-cols-[auto_1fr_auto_auto_auto_auto] items-center gap-4 border-b border-neutral-100 px-5 py-3">
            <span />
            <span className="text-11 font-semibold uppercase tracking-wider text-neutral-400">Product</span>
            <span className="text-11 font-semibold uppercase tracking-wider text-neutral-400">Seller</span>
            <span className="text-11 font-semibold uppercase tracking-wider text-neutral-400">Price</span>
            <span className="text-11 font-semibold uppercase tracking-wider text-neutral-400">Stock</span>
          ...
          <div className="divide-y divide-neutral-50">
            {products.map((product) => (
              <div key={product.id} className="grid grid-cols-[auto_1fr_auto_auto_auto_auto] items-center gap-4 px-5 py-4">
                ...
                {/* Price */}
                <span className="text-13 text-neutral-700">{formatPrice(product.price)}</span>

                {/* Stock */}
                <span className={`text-13 ${product.stock === 0 ? 'text-red-400' : product.stock <= 10 ? 'text-amber-500' : 'text-neutral-600'}`}>
                  {product.stock}
                </span>
